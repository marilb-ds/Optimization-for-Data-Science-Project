# -*- coding: utf-8 -*-
"""Recommender systems - Group 1.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1ul9LRACGjAYW0F1RLo1xuPM3RIFtIE-Q

# **ODS: Final Project**
**Recommender Systems - Group 1**

This notebook contains all three algorithms: Frank Wolfe (FW), Pairwise Frank Wolfe (PFW) and Projected Gradient Descent (PGD). All algorithms were tested in three different matrices, with different levels of sparsity, with the purpose of comparing their performance. More information about each matrix can be found along the code.
...
"""The general pipeline is mostly the same for each dataset:

- First, the data is loaded, then the normalization step is performed (in these cases, it is just the max scaling).

- Second, a small portion of the existing entries are taken as the test set.

- Third, the algorithms are applied to the given task.

- Fourth, the predictions are transformed back to the initial scale and the metrics are calculated. The metrics are the mean absolute error (MAE) and accuracy, depending on the solving task.

# 1. Lens Movies Short

The first dataset is a shortened version of the Lens Movie dataset. The original version is very sparse, in this version the top 300 active users and the top 30 most reviewed movies are taken. This dataset is selected due to the fact that it is a simple dataset (only around 43% percent of entries is missing) and a good performance is expected from it. The size allows us to perform calculations in a matter of hundreds of milliseconds and pick the optimal parameters if needed.
"""
...
The results are more than satisfactory, as the loss converged quickly and the metrics on the unseen entries are also satisfactory. To be more specific, in the 25% of cases the exact grade was predicted correctly and around 50% if calculated as if there are no intermediate .5 grades.

# 2. Jester dataset

The Jester dataset is sparser, around 78% of the entries is missing . In addition, the entries are continuous in the range [-10, 10]. It differs from most of the datasets that ususally contains discrete values. The rows correpond to users, while the columns correspond to the jokes. The entries are grades given to a joke by a user. The size of the matrix is 50,692 x 150. The expected performance should be lower compared to the Lens dataset.
"""
...
For this dataset, no classification metrics were calculated because the original scores were continuous. The MAE score is 0.36 which is also a satisfactory result. However, as can be infered from the plot, the loss converges much slower and it is about to be stucked after only a few iterations.

# 3. Amazon products

The Amazon dataset contains 827 products and numerical reviews assigned to them by 622 users. This dataset is very sparse as only 0.6% of the entries are non-empty. The choice is justified by the need to estimate the algorithm on really sparse data.
"""
...
The results are also satisfactory, given the difficulty of the task. In 41% of the cases, the precise grade was predicted correctly with the MAE score of 1.8.
"""